...
@Order(1)
@Component
public class DonationSuccessfulPaymentHandler implements UpdateHandler {
    private final QrWorkflow workflow;

    public DonationSuccessfulPaymentHandler(QrWorkflow workflow) {
    ...
    public void handle(UpdateWebhook update) {
        workflow.acceptSuccessfulDonation(update.getMessage());
    }
}
