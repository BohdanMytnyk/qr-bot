...
@Order(10)
@Component
public class DeleteQrCallbackHandler implements CallbackHandler {
    private final QrWorkflow workflow;
    private final TelegramGateway telegram;

    public DeleteQrCallbackHandler(QrWorkflow workflow, TelegramGateway telegram) {
    ...
    public boolean supports(UpdateWebhook update) {
        return update.getCallbackQuery() != null && update.getCallbackQuery().getData() != null
                && update.getCallbackQuery().getData().matches("^" + QrWorkflow.DELETE_PREFIX + "[0-9a-fA-F-]{36}$");
    }

    public void handle(UpdateWebhook update) {
        var callback = update.getCallbackQuery();
        var qrId = callback.getData().substring(QrWorkflow.DELETE_PREFIX.length());
        var result = workflow.softDelete(qrId, callback.getFrom(), callback.getMessage().getChat().getId());
        workflow.replaceNavigation(callback.getFrom(), callback.getMessage().getChat().getId(),
                callback.getMessage().getMessageId(), callback.getMessage().getText() != null, result.view());
